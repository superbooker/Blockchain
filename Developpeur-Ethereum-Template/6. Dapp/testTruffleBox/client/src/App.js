...
class App extends Component {
  ...
  runSet = async () => {
    ...
    // console.log("l'adresse est celle ci: " + transac.events.dataStored[0].returnValues.addr);
    // console.log("la data est celle ci: " + transac.events.dataStored[0].returnValues.data);
    console.log(transac);
  ...
  render() {
    if (!this.state.web3) {
      console.log("no web3 found " + this.state.accounts);
      return <div>Loading Web3, accounts, and contract...</div>;
    }
    console.log("web3 found " + this.state.accounts);
    return (
      <div className="App">
        <h1>Good to Go!</h1>
        <p>Your Truffle Box is installed and ready.</p>
        <h2>Smart Contract Example</h2>
        <CurrentUser address={this.state.accounts[0]} />
